...
class DocumentGenerator:
    ...
    def get_required_fields_with_labels(self, doc_code: str) -> Dict[str, str]:
        """يُعيد الحقول المطلوبة مع مسمياتها بالعربية لعرض النموذج في الفرونت إند"""
        template = ALL_TEMPLATES.get(doc_code)
        if not template:
            return {}
        fields = template.get("required_fields", [])
        return {f: self.FIELD_LABELS.get(f, f) for f in fields if f != "date"}
